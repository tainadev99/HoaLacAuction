...
export default class Search2 extends Component {
  ...
  reverseMoney(money) {
    return money.toString().split('.').join('');
  }
  formatMoney(money) {
    var x;
  ...
  clickLoad() {
    ...
    if (
      ...
      Alert.alert('Hãy nhập đúng định dạng số nhỏ nhất' + '\nChỉ gồm số');
    } else if (
      !isInt(this.state.maxPrice) &&
      this.state.maxPrice != null &&
      this.state.maxPrice != ''
    ) {
      Alert.alert('Hãy nhập đúng định dạng số lớn nhất' + '\nChỉ gồm số');
    } else if (
      ...
      parseFloat(this.state.minPrice) < 0 ||
      parseFloat(this.state.maxPrice) < 0
    ) {
      Alert.alert('Số nhỏ nhất và lớn nhất không được < 0');
    } else {
      this.setState(
        {
          isLoadingData: true,
            ...
            <View
              ...
              <View style={Search2Style.viewBorderMinMax}>
                <TextInput
                  ...
                  onChangeText={text => {
                    let reverse_text = text.split('.').join('');
                    this.setState({
                      minPrice: this.reverseMoney(reverse_text),
                    });
                  }}
                  value={
                    this.state.minPrice == null
                      ? ''
                      : this.formatMoney2(this.state.minPrice)
                  }
                ...
                <TextInput
                  ...
                  onChangeText={text => {
                    let reverse_text = text.split('.').join('');
                    this.setState({
                      maxPrice: this.reverseMoney(reverse_text),
                    });
                  }}
                  value={
                    this.state.maxPrice == null
                      ? ''
                      : this.formatMoney2(this.state.maxPrice)
                  }
                  keyboardType="numeric"
                  style={Search2Style.viewMinMax}
